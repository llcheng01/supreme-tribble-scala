...
class ComplexNumberTest extends FunSuite with Matchers {
  ...
  private def assertEquals(c1: ComplexNumber, c2: ComplexNumber) {
    c1.real.toDouble should be (c2.real.toDouble +- equalityEpsilon)
    c1.imaginary.toDouble should be (c2.imaginary.toDouble +- equalityEpsilon)
  }

  test("Real part of a purely real number") {
    ComplexNumber(real = 1).real.toDouble should be (1.0 +- equalityEpsilon)
  }

  test("Real part of a purely imaginary number") {
    pending
    ComplexNumber(real = 0).real.toDouble should be (0.0 +- equalityEpsilon)
  }
  ...
  test("Imaginary part of a purely real number") {
    pending
    ComplexNumber(imaginary = 0).imaginary.toDouble should be (0.0 +- equalityEpsilon)
  }

  test("Imaginary part of a purely imaginary number") {
    pending
    ComplexNumber(imaginary = 1).imaginary.toDouble should be (1.0 +- equalityEpsilon)
  }

  test("Imaginary part of a number with real and imaginary part") {
    pending
    ComplexNumber(imaginary = 2).imaginary.toDouble should be (2.0 +- equalityEpsilon)
  }
